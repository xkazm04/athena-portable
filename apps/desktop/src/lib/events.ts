/**
 * The channel events, as the panel reads them (README §3.1; `src/athena/contracts/channel.py`).
 *
 * Eight events, one stream, one decoder. The daemon writes each event's own JSON into an SSE
 * frame; this file names the shapes and does nothing else — no second encoder, because two
 * surfaces that encode a decision card separately are two surfaces that will eventually disagree
 * about what it said.
 *
