...
/**
 * Variant B — THE LEDGER. `design/ui-pass-brief.md` §2.4.
 *
 ...
 *   - THE LABELS STAY ON THE CARD, and must, because the order changes per card. The strip could
 *     hoist its labels to a legend precisely because its order never moves; this one cannot, and
 *     the two directions disagree about that on purpose.
...
export function SlideLedger({ candidate, column, offset, medians, onFocus, onOpen }: CardProps) {
  ...
  return (
    ...
    >
      <span className="bd-slide-inner">
        ...
        <span className="bd-ledger-verdict">
        ...
        <span className="bd-ledger-rows">
          {comparable ? (
            <span className="bd-ledger-head">
              <span>against the {medians.size === 0 ? 0 : column.scored} scored here</span>
            </span>
          ) : null}
          {rows.map(({ score, delta, isGap }) => (
            <span className="bd-ledger-row" key={score.criterionId} data-gap={isGap}>
              <span className="bd-ledger-weight" data-weight={score.weight} aria-hidden>
                {Array.from({ length: score.weight }, (_, i) => (
                  <i key={i} />
                ))}
              </span>
              <span className="bd-ledger-name">{score.short}</span>
              <span
                className="bd-ledger-axis"
                style={{ "--score": score.score } as CSSProperties}
                aria-hidden
              >
                {candidate.scored ? <span className="bd-ledger-fill" /> : null}
              </span>
              <span className="bd-ledger-abs">
                {candidate.scored ? `${score.score}/${SCORE_MAX}` : "—"}
              </span>
              <span
                className="bd-ledger-delta"
          ...
          {candidate.gap ? (
            <span className="bd-ledger-gapline">nothing quoted for {candidate.gap.toLowerCase()}</span>
          ) : null}
        </span>
